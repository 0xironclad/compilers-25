...
declaration:
    T_INTEGER T_ID T_SEMICOLON
    {
		...
		symbol_table[*$2] = var_data( LINE_NUMBER_VAR.first_line, integer );
        delete $2;
    }
|
    T_BOOLEAN T_ID T_SEMICOLON
    {
		...
		symbol_table[*$2] = var_data( LINE_NUMBER_VAR.first_line, boolean );
        delete $2;
    }
|
    T_TIME T_ID T_SEMICOLON
    {
        ...
        symbol_table[*$2] = var_data( LINE_NUMBER_VAR.first_line, time_type );
        delete $2;
    }
;
...
|
    T_HOUR T_OPEN expression T_CLOSE
    {
        if(time_type != *$3){
           std::stringstream ss;
           ss << "Type (" << type_name(time_type) << ") expected for hour(), got (" << type_name(*$3) << ")" << std::endl;
           error( ss.str().c_str() );
        }
        $$ = new type(integer);
        delete $3;    
...
|
    T_MINUTE T_OPEN expression T_CLOSE
    {
        if(time_type != *$3){
            std::stringstream ss;
            ss << "Type (" << type_name(time_type) << ") expected for minute(), got (" << type_name(*$3) << ")" << std::endl;
            error( ss.str().c_str() );
        }  
        $$ = new type(integer);
        delete $3;  
