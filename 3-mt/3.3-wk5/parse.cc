// Generated by Bisonc++ V6.08.00 on Sun, 02 Nov 2025 16:52:41 +0100
...
PI_ const s_productionInfo[] = 
{
     ...
     {308, 7}, // 7: declaration (T_INTEGER) ->  T_INTEGER T_ARRAY T_ID T_OPEN_SQ T_NUM T_CLOSE_SQ T_SEMICOLON
     {307, 1}, // 8: statements ->  statement
     {307, 2}, // 9: statements ->  statement statements
     {309, 2}, // 10: statement (T_SKIP) ->  T_SKIP T_SEMICOLON
     {309, 1}, // 11: statement ->  assignment
     {309, 1}, // 12: statement ->  read
     {309, 1}, // 13: statement ->  write
     {309, 1}, // 14: statement ->  branch
     {309, 1}, // 15: statement ->  loop
     {310, 4}, // 16: assignment (T_ID) ->  T_ID T_ASSIGN expression T_SEMICOLON
     {310, 6}, // 17: assignment (T_ID) ->  T_ID T_OPEN_SQ T_CLOSE_SQ T_EQ array_literal T_SEMICOLON
     {310, 7}, // 18: assignment (T_ID) ->  T_ID T_OPEN_SQ T_NUM T_CLOSE_SQ T_ASSIGN expression T_SEMICOLON
     {311, 5}, // 19: read (T_READ) ->  T_READ T_OPEN T_ID T_CLOSE T_SEMICOLON
     {311, 8}, // 20: read (T_READ) ->  T_READ T_OPEN T_ID T_OPEN_SQ T_NUM T_CLOSE_SQ T_CLOSE T_SEMICOLON
     {312, 5}, // 21: write (T_WRITE) ->  T_WRITE T_OPEN expression T_CLOSE T_SEMICOLON
     {313, 5}, // 22: branch (T_IF) ->  T_IF expression T_THEN statements T_ENDIF
     {313, 7}, // 23: branch (T_IF) ->  T_IF expression T_THEN statements T_ELSE statements T_ENDIF
     {314, 5}, // 24: loop (T_WHILE) ->  T_WHILE expression T_DO statements T_DONE
     {314, 9}, // 25: loop (T_FOR) ->  T_FOR T_ID T_ASSIGN expression T_TO expression T_DO statements T_DONE
     {314, 14}, // 26: loop (T_FOR) ->  T_FOR T_OPEN T_INTEGER T_ID T_ASSIGN expression T_SEMICOLON expression T_SEMICOLON expression T_CLOSE T_DO statements T_DONE
     {316, 2}, // 27: array_literal (T_OPEN_CURLY) ->  T_OPEN_CURLY T_CLOSE_CURLY
     {316, 3}, // 28: array_literal (T_OPEN_CURLY) ->  T_OPEN_CURLY expression_list T_CLOSE_CURLY
     {317, 1}, // 29: expression_list ->  expression
     {317, 3}, // 30: expression_list (T_COMMA) ->  expression_list T_COMMA expression
     {315, 1}, // 31: expression (T_NUM) ->  T_NUM
     {315, 1}, // 32: expression (T_TRUE) ->  T_TRUE
     {315, 1}, // 33: expression (T_FALSE) ->  T_FALSE
     {315, 1}, // 34: expression (T_ID) ->  T_ID
     {315, 1}, // 35: expression (T_TIME_LITERAL) ->  T_TIME_LITERAL
     {315, 4}, // 36: expression (T_HOUR) ->  T_HOUR T_OPEN expression T_CLOSE
     {315, 4}, // 37: expression (T_MINUTE) ->  T_MINUTE T_OPEN expression T_CLOSE
     {315, 3}, // 38: expression (T_ADD) ->  expression T_ADD expression
     {315, 3}, // 39: expression (T_SUB) ->  expression T_SUB expression
     {315, 3}, // 40: expression (T_MUL) ->  expression T_MUL expression
     {315, 3}, // 41: expression (T_DIV) ->  expression T_DIV expression
     {315, 3}, // 42: expression (T_MOD) ->  expression T_MOD expression
     {315, 3}, // 43: expression (T_LESS) ->  expression T_LESS expression
     {315, 3}, // 44: expression (T_GR) ->  expression T_GR expression
     {315, 3}, // 45: expression (T_EQ) ->  expression T_EQ expression
     {315, 3}, // 46: expression (T_AND) ->  expression T_AND expression
     {315, 3}, // 47: expression (T_OR) ->  expression T_OR expression
     {315, 2}, // 48: expression (T_NOT) ->  T_NOT expression
     {315, 3}, // 49: expression (T_OPEN) ->  T_OPEN expression T_CLOSE
     {315, 2}, // 50: expression (T_ID) ->  T_ID T_PP
     {315, 2}, // 51: expression (T_ID) ->  T_ID T_MM
     {315, 2}, // 52: expression (T_PP) ->  T_PP T_ID
     {315, 2}, // 53: expression (T_MM) ->  T_MM T_ID
     {315, 1}, // 54: expression ->  array_literal
     {315, 4}, // 55: expression (T_ID) ->  T_ID T_OPEN_SQ T_NUM T_CLOSE_SQ
     {318, 1}, // 56: program_$ ->  program
};
...
{
    { { REQ_TOKEN}, { 17} },                   
    { {       270}, { 83} }, // T_CLOSE_CURLY  
    { {       317}, { 84} }, // expression_list
    { {       315}, { 85} }, // expression     
    { {       290}, { 41} }, // T_NUM          
    { {       288}, { 42} }, // T_TRUE         
    { {       289}, { 43} }, // T_FALSE        
...
#line 76 "while.y"
        {
         std::cout << "declaration -> T_TIME T_ID(" << *vs_(-1).name << ") T_SEMICOLON" << std::endl;
         }
        break;
