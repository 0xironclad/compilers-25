// Generated by Bisonc++ V6.08.00 on Sat, 01 Nov 2025 23:16:00 +0100
...
PI_ const s_productionInfo[] = 
{
     ...
     {299, 6}, // 1: program (T_PROGRAM) ->  T_PROGRAM T_ID declarations T_BEGIN statements T_END
     {300, 0}, // 2: declarations ->  <empty>
     {300, 2}, // 3: declarations ->  declaration declarations
     {302, 3}, // 4: declaration (T_INTEGER) ->  T_INTEGER T_ID T_SEMICOLON
     {302, 3}, // 5: declaration (T_BOOLEAN) ->  T_BOOLEAN T_ID T_SEMICOLON
     {302, 3}, // 6: declaration (T_TIME) ->  T_TIME T_ID T_SEMICOLON
     {301, 1}, // 7: statements ->  statement
     {301, 2}, // 8: statements ->  statement statements
     {303, 2}, // 9: statement (T_SKIP) ->  T_SKIP T_SEMICOLON
     {303, 1}, // 10: statement ->  assignment
     {303, 1}, // 11: statement ->  read
     {303, 1}, // 12: statement ->  write
     {303, 1}, // 13: statement ->  branch
     {303, 1}, // 14: statement ->  loop
     {304, 4}, // 15: assignment (T_ID) ->  T_ID T_ASSIGN expression T_SEMICOLON
     {305, 5}, // 16: read (T_READ) ->  T_READ T_OPEN T_ID T_CLOSE T_SEMICOLON
     {306, 5}, // 17: write (T_WRITE) ->  T_WRITE T_OPEN expression T_CLOSE T_SEMICOLON
     {307, 5}, // 18: branch (T_IF) ->  T_IF expression T_THEN statements T_ENDIF
     {307, 7}, // 19: branch (T_IF) ->  T_IF expression T_THEN statements T_ELSE statements T_ENDIF
     {308, 5}, // 20: loop (T_WHILE) ->  T_WHILE expression T_DO statements T_DONE
     {308, 9}, // 21: loop (T_FOR) ->  T_FOR T_ID T_ASSIGN expression T_TO expression T_DO statements T_DONE
     {308, 14}, // 22: loop (T_FOR) ->  T_FOR T_OPEN T_INTEGER T_ID T_ASSIGN expression T_SEMICOLON expression T_SEMICOLON expression T_CLOSE T_DO statements T_DONE
     {309, 1}, // 23: expression (T_NUM) ->  T_NUM
     {309, 1}, // 24: expression (T_TRUE) ->  T_TRUE
     {309, 1}, // 25: expression (T_FALSE) ->  T_FALSE
     {309, 1}, // 26: expression (T_ID) ->  T_ID
     {309, 1}, // 27: expression (T_TIME_LITERAL) ->  T_TIME_LITERAL
     {309, 4}, // 28: expression (T_HOUR) ->  T_HOUR T_OPEN expression T_CLOSE
     {309, 4}, // 29: expression (T_MINUTE) ->  T_MINUTE T_OPEN expression T_CLOSE
     {309, 3}, // 30: expression (T_ADD) ->  expression T_ADD expression
     {309, 3}, // 31: expression (T_SUB) ->  expression T_SUB expression
     {309, 3}, // 32: expression (T_MUL) ->  expression T_MUL expression
     {309, 3}, // 33: expression (T_DIV) ->  expression T_DIV expression
     {309, 3}, // 34: expression (T_MOD) ->  expression T_MOD expression
     {309, 3}, // 35: expression (T_LESS) ->  expression T_LESS expression
     {309, 3}, // 36: expression (T_GR) ->  expression T_GR expression
     {309, 3}, // 37: expression (T_EQ) ->  expression T_EQ expression
     {309, 3}, // 38: expression (T_AND) ->  expression T_AND expression
     {309, 3}, // 39: expression (T_OR) ->  expression T_OR expression
     {309, 2}, // 40: expression (T_NOT) ->  T_NOT expression
     {309, 3}, // 41: expression (T_OPEN) ->  T_OPEN expression T_CLOSE
     {309, 2}, // 42: expression (T_ID) ->  T_ID T_PP
     {309, 2}, // 43: expression (T_ID) ->  T_ID T_MM
     {309, 2}, // 44: expression (T_PP) ->  T_PP T_ID
     {309, 2}, // 45: expression (T_MM) ->  T_MM T_ID
     {310, 1}, // 46: program_$ ->  program
};
