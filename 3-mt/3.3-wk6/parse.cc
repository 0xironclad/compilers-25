// Generated by Bisonc++ V6.08.00 on Sun, 02 Nov 2025 20:01:42 +0100
...
PI_ const s_productionInfo[] = 
{
     ...
     {299, 4}, // 14: assignment (T_ID) ->  T_ID T_ASSIGN expression T_SEMICOLON
     {300, 5}, // 15: read (T_READ) ->  T_READ T_OPEN T_ID T_CLOSE T_SEMICOLON
     {301, 5}, // 16: write (T_WRITE) ->  T_WRITE T_OPEN expression T_CLOSE T_SEMICOLON
     {302, 5}, // 17: branch (T_IF) ->  T_IF expression T_THEN statements T_ENDIF
     {302, 7}, // 18: branch (T_IF) ->  T_IF expression T_THEN statements T_ELSE statements T_ENDIF
     {303, 5}, // 19: loop (T_WHILE) ->  T_WHILE expression T_DO statements T_DONE
     {304, 1}, // 20: expression (T_NUM) ->  T_NUM
     {304, 1}, // 21: expression (T_TRUE) ->  T_TRUE
     {304, 1}, // 22: expression (T_FALSE) ->  T_FALSE
     {304, 1}, // 23: expression (T_ID) ->  T_ID
     {304, 3}, // 24: expression (T_ADD) ->  expression T_ADD expression
     {304, 3}, // 25: expression (T_SUB) ->  expression T_SUB expression
     {304, 3}, // 26: expression (T_MUL) ->  expression T_MUL expression
     {304, 3}, // 27: expression (T_DIV) ->  expression T_DIV expression
     {304, 3}, // 28: expression (T_MOD) ->  expression T_MOD expression
     {304, 3}, // 29: expression (T_LESS) ->  expression T_LESS expression
     {304, 3}, // 30: expression (T_GR) ->  expression T_GR expression
     {304, 3}, // 31: expression (T_EQ) ->  expression T_EQ expression
     {304, 3}, // 32: expression (T_AND) ->  expression T_AND expression
     {304, 3}, // 33: expression (T_OR) ->  expression T_OR expression
     {304, 2}, // 34: expression (T_NOT) ->  T_NOT expression
     {304, 3}, // 35: expression (T_OPEN) ->  T_OPEN expression T_CLOSE
     {304, 3}, // 36: expression (T_SHIFT) ->  expression T_SHIFT expression
     {304, 7}, // 37: expression (T_OPEN) ->  T_OPEN expression T_QUESTION expression T_COLON expression T_CLOSE
