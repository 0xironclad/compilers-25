// Generated by Bisonc++ V6.08.00 on Mon, 03 Nov 2025 03:19:28 +0100
...
PI_ const s_productionInfo[] = 
{
     ...
     {321, 1}, // 27: id_sequence (T_ID) ->  T_ID
     {321, 3}, // 28: id_sequence (T_COMMA) ->  id_sequence T_COMMA T_ID
     {322, 1}, // 29: expression_sequence ->  expression
     {322, 3}, // 30: expression_sequence (T_COMMA) ->  expression_sequence T_COMMA expression
     {310, 4}, // 31: assignment (T_ID) ->  T_ID T_ASSIGN expression T_SEMICOLON
     {310, 4}, // 32: assignment (T_ID) ->  T_ID T_APPENDMENT expression T_SEMICOLON
     {310, 7}, // 33: assignment (T_ID) ->  T_ID T_OPEN_SQ T_NUM T_CLOSE_SQ T_ASSIGN expression T_SEMICOLON
     {310, 6}, // 34: assignment (T_ID) ->  T_ID T_DOT T_NUM T_ASSIGN expression T_SEMICOLON
     {310, 4}, // 35: assignment (T_ASSIGN) ->  id_sequence T_ASSIGN expression_sequence T_SEMICOLON
     {316, 3}, // 36: assignment_no_semi (T_ID) ->  T_ID T_ASSIGN expression
     {311, 5}, // 37: read (T_READ) ->  T_READ T_OPEN T_ID T_CLOSE T_SEMICOLON
     {311, 7}, // 38: read (T_READ) ->  T_READ T_OPEN T_ID T_DOT T_NUM T_CLOSE T_SEMICOLON
     {317, 4}, // 39: read_no_semi (T_READ) ->  T_READ T_OPEN T_ID T_CLOSE
     {312, 5}, // 40: write (T_WRITE) ->  T_WRITE T_OPEN expression T_CLOSE T_SEMICOLON
     {318, 4}, // 41: write_no_semi (T_WRITE) ->  T_WRITE T_OPEN expression T_CLOSE
     {313, 5}, // 42: branch (T_IF) ->  T_IF expression T_THEN statements T_ENDIF
     {313, 7}, // 43: branch (T_IF) ->  T_IF expression T_THEN statements T_ELSE statements T_ENDIF
     {314, 5}, // 44: loop (T_WHILE) ->  T_WHILE expression T_DO statements T_DONE
     {323, 2}, // 45: list (T_OPEN_SQ) ->  T_OPEN_SQ T_CLOSE_SQ
     {323, 3}, // 46: list (T_OPEN_SQ) ->  T_OPEN_SQ list_sequence T_CLOSE_SQ
     {320, 1}, // 47: expression (T_NUM) ->  T_NUM
     {320, 1}, // 48: expression (T_TRUE) ->  T_TRUE
     {320, 1}, // 49: expression (T_FALSE) ->  T_FALSE
     {320, 1}, // 50: expression (T_ID) ->  T_ID
     {320, 3}, // 51: expression (T_ADD) ->  expression T_ADD expression
     {320, 3}, // 52: expression (T_SUB) ->  expression T_SUB expression
     {320, 3}, // 53: expression (T_MUL) ->  expression T_MUL expression
     {320, 3}, // 54: expression (T_DIV) ->  expression T_DIV expression
     {320, 3}, // 55: expression (T_MOD) ->  expression T_MOD expression
     {320, 3}, // 56: expression (T_LESS) ->  expression T_LESS expression
     {320, 3}, // 57: expression (T_GR) ->  expression T_GR expression
     {320, 3}, // 58: expression (T_EQ) ->  expression T_EQ expression
     {320, 3}, // 59: expression (T_AND) ->  expression T_AND expression
     {320, 3}, // 60: expression (T_OR) ->  expression T_OR expression
     {320, 2}, // 61: expression (T_NOT) ->  T_NOT expression
     {320, 3}, // 62: expression (T_OPEN) ->  T_OPEN expression T_CLOSE
     {320, 3}, // 63: expression (T_SHIFT) ->  expression T_SHIFT expression
     {320, 7}, // 64: expression (T_OPEN) ->  T_OPEN expression T_QUESTION expression T_COLON expression T_CLOSE
     {320, 4}, // 65: expression (T_ID) ->  T_ID T_OPEN_SQ T_NUM T_CLOSE_SQ
     {320, 1}, // 66: expression ->  list
     {320, 3}, // 67: expression (T_PLUS_PLUS) ->  expression T_PLUS_PLUS expression
     {320, 3}, // 68: expression (T_ID) ->  T_ID T_DOT T_NUM
     {324, 1}, // 69: program_$ ->  program
};
...
{
    { { REQ_TOKEN}, {  8} },                      
    { {       315}, { 40} }, // statement_no_semi 
    { {       316}, { 41} }, // assignment_no_semi
    { {       317}, { 42} }, // read_no_semi      
    { {       318}, { 43} }, // write_no_semi     
    { {       287}, { 44} }, // T_ID              
    { {       271}, { 45} }, // T_READ            
    { {       272}, { 46} }, // T_WRITE           
    { {         0}, {  0} },                      
