// Generated by Bisonc++ V6.08.00 on Mon, 03 Nov 2025 01:14:52 +0100
...
PI_ const s_productionInfo[] = 
{
     ...
     {304, 3}, // 4: declaration (T_SEMICOLON) ->  type variable_sequence T_SEMICOLON
     {304, 4}, // 5: declaration (T_LIST) ->  type T_LIST T_ID T_SEMICOLON
     {305, 1}, // 6: type (T_INTEGER) ->  T_INTEGER
     {305, 1}, // 7: type (T_BOOLEAN) ->  T_BOOLEAN
     {306, 1}, // 8: variable_sequence (T_ID) ->  T_ID
     {306, 3}, // 9: variable_sequence (T_COMMA) ->  variable_sequence T_COMMA T_ID
     {303, 1}, // 10: statements ->  statement
     {303, 2}, // 11: statements ->  statement statements
     {307, 2}, // 12: statement (T_SKIP) ->  T_SKIP T_SEMICOLON
     {307, 1}, // 13: statement ->  assignment
     {307, 1}, // 14: statement ->  read
     {307, 1}, // 15: statement ->  write
     {307, 1}, // 16: statement ->  branch
     {307, 1}, // 17: statement ->  loop
     {307, 6}, // 18: statement (T_OPEN_SQ) ->  T_OPEN_SQ statement_no_semi T_PARALLEL statement_no_semi T_CLOSE_SQ T_SEMICOLON
     {313, 1}, // 19: statement_no_semi ->  assignment_no_semi
     {313, 1}, // 20: statement_no_semi ->  read_no_semi
     {313, 1}, // 21: statement_no_semi ->  write_no_semi
     {317, 1}, // 22: list_sequence ->  expression
     {317, 3}, // 23: list_sequence (T_COMMA) ->  list_sequence T_COMMA expression
     {308, 4}, // 24: assignment (T_ID) ->  T_ID T_ASSIGN expression T_SEMICOLON
     {308, 4}, // 25: assignment (T_ID) ->  T_ID T_APPENDMENT expression T_SEMICOLON
     {308, 7}, // 26: assignment (T_ID) ->  T_ID T_OPEN_SQ T_NUM T_CLOSE_SQ T_ASSIGN expression T_SEMICOLON
     {314, 3}, // 27: assignment_no_semi (T_ID) ->  T_ID T_ASSIGN expression
     {309, 5}, // 28: read (T_READ) ->  T_READ T_OPEN T_ID T_CLOSE T_SEMICOLON
     {315, 4}, // 29: read_no_semi (T_READ) ->  T_READ T_OPEN T_ID T_CLOSE
     {310, 5}, // 30: write (T_WRITE) ->  T_WRITE T_OPEN expression T_CLOSE T_SEMICOLON
     {316, 4}, // 31: write_no_semi (T_WRITE) ->  T_WRITE T_OPEN expression T_CLOSE
     {311, 5}, // 32: branch (T_IF) ->  T_IF expression T_THEN statements T_ENDIF
     {311, 7}, // 33: branch (T_IF) ->  T_IF expression T_THEN statements T_ELSE statements T_ENDIF
     {312, 5}, // 34: loop (T_WHILE) ->  T_WHILE expression T_DO statements T_DONE
     {319, 2}, // 35: list (T_OPEN_SQ) ->  T_OPEN_SQ T_CLOSE_SQ
     {319, 3}, // 36: list (T_OPEN_SQ) ->  T_OPEN_SQ list_sequence T_CLOSE_SQ
     {318, 1}, // 37: expression (T_NUM) ->  T_NUM
     {318, 1}, // 38: expression (T_TRUE) ->  T_TRUE
     {318, 1}, // 39: expression (T_FALSE) ->  T_FALSE
     {318, 1}, // 40: expression (T_ID) ->  T_ID
     {318, 3}, // 41: expression (T_ADD) ->  expression T_ADD expression
     {318, 3}, // 42: expression (T_SUB) ->  expression T_SUB expression
     {318, 3}, // 43: expression (T_MUL) ->  expression T_MUL expression
     {318, 3}, // 44: expression (T_DIV) ->  expression T_DIV expression
     {318, 3}, // 45: expression (T_MOD) ->  expression T_MOD expression
     {318, 3}, // 46: expression (T_LESS) ->  expression T_LESS expression
     {318, 3}, // 47: expression (T_GR) ->  expression T_GR expression
     {318, 3}, // 48: expression (T_EQ) ->  expression T_EQ expression
     {318, 3}, // 49: expression (T_AND) ->  expression T_AND expression
     {318, 3}, // 50: expression (T_OR) ->  expression T_OR expression
     {318, 2}, // 51: expression (T_NOT) ->  T_NOT expression
     {318, 3}, // 52: expression (T_OPEN) ->  T_OPEN expression T_CLOSE
     {318, 3}, // 53: expression (T_SHIFT) ->  expression T_SHIFT expression
     {318, 7}, // 54: expression (T_OPEN) ->  T_OPEN expression T_QUESTION expression T_COLON expression T_CLOSE
     {318, 4}, // 55: expression (T_ID) ->  T_ID T_OPEN_SQ T_NUM T_CLOSE_SQ
     {318, 1}, // 56: expression ->  list
     {318, 3}, // 57: expression (T_PLUS_PLUS) ->  expression T_PLUS_PLUS expression
     {320, 1}, // 58: program_$ ->  program
};
...
{
    { { REQ_TOKEN}, {  8} },                      
    { {       313}, { 34} }, // statement_no_semi 
    { {       314}, { 35} }, // assignment_no_semi
    { {       315}, { 36} }, // read_no_semi      
    { {       316}, { 37} }, // write_no_semi     
    { {       287}, { 38} }, // T_ID              
    { {       271}, { 39} }, // T_READ            
    { {       272}, { 40} }, // T_WRITE           
    { {         0}, {  0} },                      
