...
|
    T_TIME T_ID T_SEMICOLON
    {
        ...
        symbol_table[*$2] = var_data( d_loc_.first_line, time_type );
        delete $2;
    }
;
...
assignment:
    T_ID T_ASSIGN expression T_SEMICOLON
    {
		...
		{
		   std::stringstream ss;
		   ss << d_loc_.first_line << ": Type error." << std::endl;
		   error( ss.str().c_str() );
		}
        if(symbol_table[*$1].decl_type == time_type && symbol_table[*$1].assigned){
            std::stringstream ss;
            ss << d_loc_.first_line << ": Variable already assigned." << std::endl;
            error( ss.str().c_str() );
        }
